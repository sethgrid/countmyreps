...
function send_email_success($user_email, $addressed_to, $subject, $time, $Log=null, $MyCurlRequest=null){
	...
	if ($office == ""){
		$msg .= "You do not have your office set. Send an email with your office name in the subject. Choose from oc, denver, boulder, euro, providence, san francisco, or new york. This should be sent in ints own email.\n";
	} else {
	...
	$msg .= "The totals are as follows: OC has ".$data['oc']['total'].", Boulder has ".$data['boulder']['total'].", Denver has ".$data['denver']['total'].", San Francisco has ".$data['san_francisco']['total'].", Providence has ".$data['providence']['total'].", New York has ".$data['new_york']['total'].", and the Euro team has ".$data['euro']['total'].".\n";
