...
class DataStore
{
    ...
    function get_count_by_office($office){
        $query = $this->db->prepare("select count(*) from (select u.email from user as u join reps as r on r.user_id=u.id where r.created_at > \"$this->start\" and office=:office group by u.email) as e where 1");
